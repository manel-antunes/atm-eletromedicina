...
async function enviarEmail(destinatarios: string[], subject: string, html: string) {
  await resend.emails.send({
    from: 'ATM Eletromedicina <onboarding@resend.dev>',
    to: destinatarios,
    subject,
    html,
  })
  ...
  cronJob = cron.schedule(expressao, async () => {
    ...
    try {
      await enviarEmail(
        ...
        `⚠️ ATM — Alertas de calibração · ${new Date().toLocaleDateString('pt-PT')}`,
        gerarHTML(ultimosAlertasCache)
      )
      await pool.query(
...
app.post('/api/enviar-alertas', async (req, res) => {
  ...
  try {
    await enviarEmail(
      destinos,
      `⚠️ ATM — ${alertas.filter((a: Alerta) => a.estado === 'vencido').length} vencidas · ${new Date().toLocaleDateString('pt-PT')}`,
      gerarHTML(alertas)
    )
    await pool.query(
      `INSERT INTO historico_emails (data, hora, destinatarios, total_alertas, vencidas, urgentes, em_breve, sucesso)
    ...
    await pool.query(
      `INSERT INTO historico_emails (data, hora, destinatarios, total_alertas, vencidas, urgentes, em_breve, sucesso, erro)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [new Date().toLocaleDateString('pt-PT'), new Date().toLocaleTimeString('pt-PT'),
       JSON.stringify(destinos), 0, 0, 0, 0, false, String(err)]
    )
    res.status(500).json({ erro: String(err) })
  }
})
...
app.post('/api/descricao-ia', async (req, res) => {
  ...
  const basePadrao = [
    `**Função principal**\nEquipamento de teste e medição especializado da Unidade de Eletromedicina da ATM.\n\n**Aplicações clínicas**\nIntegrado no programa de manutenção preventiva hospitalar.\n\n**Normas aplicáveis**\nIEC 60601-1 e normas específicas da categoria.\n\n**Periodicidade de calibração**\nCalibração anual obrigatória por entidade acreditada pelo IPAC.`,
  ]
