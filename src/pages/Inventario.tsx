...
export default function Inventario({ equipamentos, onVerDetalhe }: Props) {  const [pesquisa, setPesquisa] = useState('')
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtrados.length === 0 ? (
              ...
              filtrados.map(eq => {
                const loc = eq.localizacao?.toLowerCase() ?? ''
                const emEletro = loc.includes('hprt') || loc.includes('fixo') || loc.includes('eletro')
                return (
<tr key={eq.id} onClick={() => onVerDetalhe(eq)} className="border-b border-gray-50 hover:bg-blue-50 transition-colors cursor-pointer">                    <td className="px-4 py-3 font-mono text-xs text-gray-500">{eq.numeroSAP}</td>
