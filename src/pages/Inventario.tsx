    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-gray-50">
          {filtrados.length === 0 ? (
            <p className="py-8 text-center text-sm text-gray-400">Nenhum equipamento encontrado</p>
          ) : filtrados.map(eq => {
              ...
              <div key={eq.id} onClick={() => onVerDetalhe(eq)} className="p-4 hover:bg-red-50/40 cursor-pointer">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-mono text-gray-400">{eq.numeroSAP}</p>
                  </div>
                  {cedencia && (
                    <span style={{ background: '#fff7ed', color: '#9a3412', fontSize: 10, fontWeight: 700, padding: '2px 7px', flexShrink: 0 }}>Cedido</span>
                  )}
                </div>
                <div className="mt-1 flex flex-wrap gap-x-4 gap-y-0.5">
                  <p className="text-xs text-gray-500">{eq.marca} {eq.modelo}</p>
                  {eq.numeroSerie && <p className="text-xs font-mono text-gray-400">{eq.numeroSerie}</p>}
                  {eq.localizacao && <p className="text-xs text-gray-500">{cedencia ? cedencia.destino : eq.localizacao}</p>}
                </div>
              </div>
            )
          })}
