...
export default function Login({ onLogin }: Props) {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (id) localStorage.setItem('atm_user_id', String(id))
    ...
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', background: '#0A0F1E', display: 'flex' }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 36, background: '#C0001A', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
          <div style={{ display: 'flex', animation: 'marquee 20s linear infinite', whiteSpace: 'nowrap' }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <span key={i} style={{ fontFamily: "'Noto Sans'", fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.8)', letterSpacing: '0.2em', textTransform: 'uppercase', padding: '0 32px' }}>
      ...
      >
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 32 }} aria-label="Formulário de autenticação">
          <div>
            <label htmlFor="login-username" style={{ display: 'block', fontFamily: "'Noto Sans'", fontSize: 9, fontWeight: 500, color: focusedField === 'user' ? '#C0001A' : '#999', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 8, transition: 'color 0.3s' }}>
              Utilizador
            </label>
            <input
              id="login-username"
          ...
          <div>
            <label htmlFor="login-password" style={{ display: 'block', fontFamily: "'Noto Sans'", fontSize: 9, fontWeight: 500, color: focusedField === 'pass' ? '#C0001A' : '#999', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 8, transition: 'color 0.3s' }}>
              Password
            </label>
            <input
              id="login-password"
          ...
          {erro && (
            <div id="login-erro" role="alert" style={{ display: 'flex', alignItems: 'center', gap: 10, borderLeft: '2px solid #C0001A', paddingLeft: 12 }}>
              <span style={{ fontFamily: "'Noto Sans'", fontSize: 12, color: '#C0001A', fontWeight: 400 }}>{erro}</span>
            </div>
          )}
        ...
        <div style={{ position: 'absolute', bottom: 40, left: 56, right: 56 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p className="footer-text" style={{ fontFamily: "'Noto Sans'", fontSize: 10, color: '#999', margin: 0, letterSpacing: '0.05em' }}>ATM Manutenção Total</p>
            <p className="footer-text" style={{ fontFamily: "'Noto Sans'", fontSize: 10, color: '#999', margin: 0, letterSpacing: '0.05em' }}>Porto · 2026</p>
          </div>
        </div>
      </div>
    </div>
